...
int main(int argc, char *argv[]){
	...
	for( int numEnv = 0; numEnv < 1000; numEnv++){
		...
		OpenRAVE::KinBodyPtr table= env->GetKinBody("wall");

		OpenRAVE::Transform t = table->GetTransform();
		OpenRAVE::Vector v = t*OpenRAVE::Vector(-.2,0,0);
		...
		if( x != -1){
			...
			envfilename << std::setfill('0') << std::setw(5) << currentIndex << "w.zae";
			...
			resultfilename << std::setfill('0') << std::setw(5) << currentIndex << "_resultwall";
